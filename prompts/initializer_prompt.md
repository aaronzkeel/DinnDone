...
This project uses **Design-OS** for specifications and design assets. Read these files in order:

1. `product-plan/product-overview.md` — Product specification (SINGLE SOURCE OF TRUTH)
2. `product-plan/instructions/incremental/*.md` — All milestone files (01-foundation through 05-notifications)
3. `product-plan/sections/*/README.md` — Detailed section requirements
4. `product-plan/sections/*/tests.md` — Test-writing instructions for each section

**Design Assets Available:**
- `product-plan/design-system/` — Colors, fonts, tokens
- `product-plan/data-model/` — TypeScript types and entity relationships
- `product-plan/shell/` — App shell components (header, nav, layout)
- `product-plan/sections/[name]/components/` — Pre-built React components
- `product-plan/sections/[name]/*.png` — Visual references (screenshots)

---

## REQUIRED FEATURE COUNT

**CRITICAL:** You must create exactly **200** features using the `feature_create_bulk` tool.

This number is specified in `product-plan/product-overview.md` and must be followed precisely.

## FEATURE ORGANIZATION (Design-OS Milestones)

Organize features by the 5 milestones from `product-plan/instructions/incremental/`:

| Milestone | File | Features | Dependencies |
|-----------|------|----------|--------------|
| 01-Foundation | `01-foundation.md` | ~20 | None (run first) |
| 02-Grocery List | `02-grocery-list.md` | ~45 | Foundation |
| 03-Meal Helper | `03-meal-helper.md` | ~45 | Foundation |
| 04-Weekly Planning | `04-weekly-planning.md` | ~45 | Foundation |
| 05-Notifications | `05-notifications.md` | ~45 | Foundation |

**WIDE GRAPH PATTERN:** Sections 02-05 all depend on Foundation but NOT on each other.
This means 4 sections can be built in parallel once Foundation is complete.
...
- Feature count must match the `feature_count` specified in `product-plan/product-overview.md`
...
Base the script on the technology stack specified in `product-plan/product-overview.md`.
...
Set up the basic project structure based on `product-plan/product-overview.md`.

**IMPORTANT:** Copy the pre-built components from Design-OS:
- `product-plan/shell/components/` → Copy to your app's components directory
- `product-plan/sections/[name]/components/` → Copy as you implement each section
- `product-plan/data-model/types.ts` → Copy to your types directory

These components are ready-to-use. DO NOT redesign or restyle them.
Your job is to wire them up to real API calls and routing.
