...
# 3. Read progress notes from previous sessions (last 500 lines to avoid context overflow)
...
```
# 5. Get progress statistics (passing/total counts)
Use the feature_get_stats tool

# 6. Get the next feature to work on
Use the feature_get_next tool
```

**Understanding the project:** This project uses Design-OS for specifications.
- Product spec: `product-plan/product-overview.md`
- Design assets: `product-plan/sections/[section-name]/`
...
### STEP 3.5: CHECK FOR DESIGN ASSETS (CRITICAL)

**Before writing any code**, check if design assets exist for this feature.

Based on the feature's section (grocery-list, meal-helper, weekly-planning, notifications),
look in `product-plan/sections/[section-name]/`:

| Asset | Location | How to Use |
|-------|----------|------------|
| Components | `components/*.tsx` | Copy to your app, wire up props — DO NOT redesign |
| Types | `types.ts` | Import for type safety |
| Sample data | `sample-data.json` | Use during development, replace with real API |
| Screenshots | `*.png` | Visual reference — match exactly |
| Tests | `tests.md` | Write failing tests FIRST (TDD approach) |

**CRITICAL RULES:**
- Use pre-built components AS-IS — do not redesign or restyle
- Components are props-based — wire up callbacks to your API/routing
- Match the screenshots exactly for visual verification
- Follow `tests.md` instructions for test-driven development

### STEP 4: IMPLEMENT THE FEATURE

Implement the chosen feature thoroughly:

1. **Check for pre-built components first** (Step 3.5)
2. Write the code (frontend and/or backend as needed)
3. Test manually using browser automation (see Step 5)
...
- Zero console errors
- Polished UI matching the screenshots in `product-plan/sections/`
...
- **Pre-built components used as-is (not redesigned)**
