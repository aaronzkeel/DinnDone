import { Clock, ChefHat, Users } from 'lucide-react'
import type { PlannedMeal, HouseholdMember } from '../types'

interface DayCardProps {
  meal: PlannedMeal
  householdMembers: HouseholdMember[]
  isToday: boolean
  onTap?: () => void
}

const effortDots = {
  'super-easy': 1,
  'middle': 2,
  'more-prep': 3,
}

const effortLabels = {
  'super-easy': 'Super Easy',
  'middle': 'Medium',
  'more-prep': 'More Prep',
}

export function DayCard({ meal, householdMembers, isToday, onTap }: DayCardProps) {
  const cook = householdMembers.find((m) => m.id === meal.assignedCookId)
  const eaters = householdMembers.filter((m) => meal.eaterIds.includes(m.id))
  const totalTime = meal.prepTime + meal.cookTime

  // Format date as "Mon 15"
  const formatDate = (dateStr: string) => {
    const date = new Date(dateStr)
    const day = date.toLocaleDateString('en-US', { weekday: 'short' })
    const num = date.getDate()
    return { day, num }
  }

  const { day, num } = formatDate(meal.date)

  return (
    <button
      onClick={onTap}
      className={`
        w-full text-left p-4 rounded-xl transition-colors
        ${isToday
          ? 'bg-yellow-50 dark:bg-yellow-950 border-2 border-yellow-300 dark:border-yellow-700'
          : 'bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 hover:border-yellow-300 dark:hover:border-yellow-600'
        }
        ${meal.isUnplanned ? 'opacity-60' : ''}
      `}
    >
      <div className="flex gap-3">
        {/* Date column */}
        <div className={`
          flex flex-col items-center justify-center w-12 flex-shrink-0
          ${isToday ? 'text-yellow-600 dark:text-yellow-400' : 'text-stone-400 dark:text-stone-500'}
        `}>
          <span className="text-xs font-medium uppercase">{day}</span>
          <span className="text-xl font-bold">{num}</span>
        </div>

        {/* Meal info */}
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <h3 className={`
              font-semibold truncate
              ${meal.isUnplanned
                ? 'text-stone-400 dark:text-stone-500 italic'
                : 'text-stone-800 dark:text-stone-100'
              }
            `}>
              {meal.isUnplanned ? 'Unplanned' : meal.mealName}
            </h3>

            {/* Effort indicator */}
            {!meal.isUnplanned && (
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <span className="text-xs font-medium text-stone-500 dark:text-stone-400">
                  {effortLabels[meal.effortTier]}
                </span>
                <div className="flex gap-0.5">
                  {Array.from({ length: 3 }).map((_, i) => (
                    <div
                      key={i}
                      className={`
                        w-2 h-2 rounded-full
                        ${i < effortDots[meal.effortTier]
                          ? 'bg-yellow-500 dark:bg-yellow-400'
                          : 'bg-stone-200 dark:bg-stone-600'
                        }
                      `}
                    />
                  ))}
                </div>
              </div>
            )}
          </div>

          {!meal.isUnplanned && (
            <div className="flex items-center gap-3 mt-2 text-sm text-stone-600 dark:text-stone-400">
              {/* Time */}
              <div className="flex items-center gap-1.5">
                <Clock size={14} />
                <span>{totalTime}m</span>
              </div>

              {/* Cook */}
              {cook && (
                <div className="flex items-center gap-1.5">
                  <ChefHat size={14} />
                  <span>{cook.name}</span>
                </div>
              )}

              {/* Eaters count */}
              <div className="flex items-center gap-1.5">
                <Users size={14} />
                <span>{eaters.length}</span>
              </div>
            </div>
          )}
        </div>
      </div>
    </button>
  )
}
