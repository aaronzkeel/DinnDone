...
**Verified:** ⬜ Waiting for user confirmation
...
1. **Made current meal clickable** - Added `onViewMealDetails` prop to EditDayModal
   ...
   - Shows "Tap to see ingredients & directions →" hint
...
**Problem:**
- Viewing meal details from Plan page required 2 clicks (day card → Edit modal → click current meal)
...
**Problem:**
- When swapping a meal (e.g., Quesadillas → Homemade Pizza), only the name and metadata changed
...
- `src/types/weekly-planning.ts`:
  - `PlannedMeal.ingredients` → `Ingredient[]`
  - `MealAlternative.ingredients` → `Ingredient[]`
  - Removed dead fields: `isUnplanned`, `briefInstructions`

- `src/types/meal-helper.ts`:
  - `PlannedMealSummary.ingredients` → `Ingredient[]`
  - `MealSuggestion.ingredients` → `Ingredient[]`
...
**Problem:** Users can spam AI requests → huge API bills.
...
### Medium Priority Items - Fixes Applied

#### 1. AI Prompts Duplication - FIXED
**Files Created:**
- `convex/aiPrompts.ts` - Shared prompt templates and helpers

**Details:**
- Extracted effort tier rules, cleanup ratings, ingredient/step counts
- Created `buildWeekPlanSystemPrompt()`, `buildAlternativesSystemPrompt()` helpers
- Created `cleanJsonResponse()` helper for parsing
- Removed ~200 lines of duplicate prompt content

#### 2. Magic Numbers - FIXED
**Files Created:**
- `src/lib/constants.ts` - Named constants with explanatory comments

**Constants Added:**
- `CHAT_HISTORY_LIMIT = 10`
- `CHAT_MAX_TOKENS = 300`
- `RECENT_MEALS_LOOKBACK_DAYS = 14`
- Debounce timings (1000-2000ms)
- UI truncation limits

**Files Updated:**
- `src/app/page.tsx`, `src/hooks/useWeekPlanning.ts`, `convex/ai.ts`

#### 3. handleSendMessage Nesting - IMPROVED
**Status:** Partially addressed by hooks extraction
- Logic now lives in `usePlanningAI` hook
- Further breakdown possible but lower priority

#### 4. Input Validation - FIXED
**Files Modified:**
- `convex/weekPlans.ts` - Meal name, ingredient, step length limits
- `convex/groceryItems.ts` - Item name, quantity, category limits

**Limits Added:**
- Meal/item names: 200 chars max
- Quantities: 100 chars max
- Steps: 1000 chars max

#### 5. Seed Function Protection - FIXED
**Files Modified:**
- `convex/weekPlans.ts` - `seedSampleWeekPlan` protected
- `convex/admin.ts` - `resetAllData` protected

**Protection:** Requires `ALLOW_SEED_DATA=true` environment variable in Convex dashboard
